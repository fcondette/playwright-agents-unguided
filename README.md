...
This repository is the **unguided baseline** of a two-part experiment measuring
how much guidance AI agents need to produce usable Playwright tests. Everything
under `tests/` was generated by the agent pipeline with no architectural
guidance, and is kept close to as produced rather than repaired: no page
objects, no fixtures, and login duplicated across all 24 spec files. Under
`--repeat-each=3` roughly a third of the suite is non-deterministic.
...
| `spec/`              | destination for generated test plans       |
...
- **MCP server approval** — choose _"Use this MCP server"_, not _"Use this and
  all future MCP servers in this project"_. Re-running `init-agents` can rewrite
...
The plan lands in `spec/` as Markdown. **Read it before generating anything.**
